...
import cn.siques.backend.dto.model.CodeEnum;
...
public class UserController {
    ...
    public Result codeLogin(@RequestBody LoginDto loginDto,HttpServletRequest request){
        ...
        if(validateCodeService.validateCode(phoneNum,verifyCode)){
    ...
    public Result sendCode(@RequestParam String phoneNumber){
        validateCodeService.sendCode(phoneNumber, String.valueOf(Math.random()).substring(2, 8));
       return Result.succeed("发送成功");
    }


    @PostMapping("/oauth/git")
    public String oauth(@RequestBody Map<String,String> map) throws IOException {
          return  validateCodeService.oauthGit(map);
    }

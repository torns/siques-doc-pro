    ...
    <div class="container h-100">
      <el-row type="flex" class="pt-4">
            ...
            <ul>
              <div v-for="(post, index) in collections.posts" :key="index">
                <div class="d-flex">
                  ...
                  <div>
                    <div class="fs-lg">
                      <router-link :to="`/p/${post.id}`" tag="a" class="hoverlink ">
                        {{ post.title }}
                      </router-link>
                    </div>
                    <div class="d-flex ai-baseline py-2">
                      ...
                      <div @click="showBookmark(post.id)" class="text-gray hoverlink fs-xm point">
        ...
        <el-col :xs="24" :sm="24" :md="6" :lg="6" :xl="6" class="hidden-sm-and-down pl-2"
          ><div v-if="collections.posts">{{ collections.posts.length }}篇文章</div></el-col
        >
...
export default class collection extends Vue {
  ...
  async bookmarkPost() {
    await this.$http.get(`/bookmarks?postId=${this.postId}&bookmarkId=${this.checkList}`)
    this.$notify({
      type: 'success',
      message: '收藏成功',
      title: '成功'

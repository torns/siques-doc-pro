  ...
  <div class="sidebar">
    <!-- <div v-if="data" class="shadow-1" style="height:204px;background-color:white;">
      <div class="px-4 py-4">
        <div class="d-flex">
          <div class="pr-3">
            <div class="d-flex bg-light border-radius jc-center ai-center" style="height:65px;width:65px;">
              <i v-if="!collection.cover" class="opacity80 fs-ll iconfont icon-zhuanlan"></i>
              <img v-else :src="collection.cover" class=" w-100 h-100" style="object-fit: cover;" alt="" />
            </div>
          </div>
          <div class="d-flex flex-column jc-between">
            ...
            <div style="width: fit-content;" class="fs-xm bg-blue text-white border-radius px-1 ">
              用户专栏
            </div>
          </div>
        </div>
        ...
        <div class="pt-4">
          <el-button @click="followCollection" size="small" type="primary">{{ isInterest ? '已关注' : '关注专栏' }}</el-button>
      ...
      <!-- <div class="d-none d-lg-block  bg-white mt-3 px-2 shadow-2" style="min-height:200px;width:300px">
        <div v-if="data" class="py-2 text-center">
          ...
          <nuxt-link :to="`/u/${data.user.id}`"
            ><el-avatar :size="100" :src="data.user.avator[0] ? data.user.avator[0].url : ''" class="shadow-1 hover-5 point" style="background-color: white ;border: 1px solid #de7d7d;padding: 3px;">
              <img src="~/static/avator.jpg" /> </el-avatar
          ></nuxt-link>
          <div style="font-weight:600" class="py-2">{{ data.user.name }}</div>
          <div class="pb-2">
            {{ data.user.description ? data.user.description : '也许将来作者会在这里留下一点痕迹' }}
          </div>
          <div class="point py-2">
            <a target="_blank" href="mqqwpa://im/chat?chat_type=wpa&uin=943452349&version=1&src_type=web&web_src=oicqzone.com"> <i class="iconfont icon-qq text-blue fs-xl"> </i></a>
